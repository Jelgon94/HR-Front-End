...
function App() {
  ...
  const startConversation = async () => {
    try {
      // Start a new session
      const sessionResponse = await axios.post('https://45.63.42.206:5000/api/start_session', {
      ...
      const response = await axios.get('https://45.63.42.206:5000/api/initial_question', {
        params: { session_id },
      });
      ...
      if (speech_file_url) {
        await playAudio(`https://45.63.42.206:5000${speech_file_url}`);
      } else {
  ...
  const handleSendAudio = async (audioBlob) => {
    ...
    try {
      const response = await axios.post('https://45.63.42.206:5000/api/conversation', formData, {
  ...
  const stopConversation = async () => {
    try {
      const response = await axios.post('https://45.63.42.206:5000/api/stop_conversation', {
        session_id: sessionId,
      });
      const { summary_file } = response.data;

      setConversationFinished(true);
      setSummaryFileUrl(`https://45.63.42.206:5000${summary_file}`);
